...
The table was verified empty before this revision, when Google sign-in had not yet been enabled. Guest samples are unchanged. Old developer-only advanced-level queues are not migrated to the redesigned catalog; after real account rollout, catalog changes will need versioned attempts.

## Google setup follow-up

The owner explicitly approved saving `https://beatfirst-ceech.vercel.app/api/auth/callback/google` in the existing Google client. Google Cloud confirmed “OAuth client saved” after the first attempt returned a transient error. No Google scopes, credentials, or other client settings changed. The owner selected `ceechhsu@gmail.com` for the browser verification. A direct Google authorization request with the exact client ID and callback now displays the normal account chooser for `beatfirst-ceech.vercel.app`, confirming that Google accepts the return address. No account was selected in this diagnostic flow and its tab was closed.

The full preview test was blocked first by automatic review requiring an explicit account choice; the owner supplied it, resolving that approval. Vercel’s separate Google account chooser then remained in its loading state after the selection and one refresh/retry. Full BeatFirst browser login, guest transfer, and cross-device persistence are therefore still unverified; the previously tested deployed progress API continues to have its recorded evidence. The owner should refresh the preview and retry sign-in on their phone. No gameplay code or deployment was changed for this configuration fix.
