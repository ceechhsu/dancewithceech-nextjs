-- One official roll call per class/date. Preserve originals in private audit storage.
lock table public.attendance_meetings, public.attendance_roster, public.attendance_records,
 public.attendance_corrections, public.attendance_change_requests in share row exclusive mode;
alter table public.attendance_meetings add column merged_into uuid references public.attendance_meetings(id);
alter table public.attendance_meetings add column reopen_count integer not null default 0;
create table public.attendance_roll_call_merge_audit (
 id uuid primary key default gen_random_uuid(), class_id uuid not null,
 meeting_date date not null, canonical_id uuid not null references attendance_meetings(id),
 originals jsonb not null, created_at timestamptz not null default clock_timestamp()
);
alter table public.attendance_roll_call_merge_audit enable row level security;
revoke all on public.attendance_roll_call_merge_audit from public,anon,authenticated;
grant select,insert on public.attendance_roll_call_merge_audit to service_role;
create trigger attendance_merge_audit_immutable before update or delete on public.attendance_roll_call_merge_audit
 for each row execute function public.attendance_immutable_audit();

-- Fail closed if a conflicting manual absence needs an instructor decision.
do $$
begin
 if exists (
  select 1 from attendance_records r join attendance_meetings m on m.id=r.meeting_id
  where m.status<>'cancelled'
  group by m.class_id,m.meeting_date,r.enrollment_id
  having count(distinct m.id)>1 and bool_or(r.status='present') and bool_or(r.status='absent' and r.source='manual')
 ) then raise exception 'Duplicate roll calls contain conflicting manual corrections; instructor review required'; end if;
 if exists(select 1 from attendance_meetings where status='open') then
  raise exception 'End open attendance windows before consolidating roll calls';
 end if;
end $$;

alter table public.attendance_records disable trigger attendance_check_in_time;
do $$
declare g record; ids uuid[]; canonical uuid; person record; chosen attendance_records; earliest timestamptz;
begin
 for g in select class_id,meeting_date from attendance_meetings where status<>'cancelled'
  group by class_id,meeting_date having count(*)>1 loop
  select array_agg(id order by opened_at,id) into ids from attendance_meetings
   where class_id=g.class_id and meeting_date=g.meeting_date and status<>'cancelled';
  canonical:=ids[1];
  insert into attendance_roll_call_merge_audit(class_id,meeting_date,canonical_id,originals)
  select g.class_id,g.meeting_date,canonical,jsonb_build_object(
   'meetings',(select jsonb_agg(to_jsonb(m)) from attendance_meetings m where id=any(ids)),
   'rosters',(select jsonb_agg(to_jsonb(r)) from attendance_roster r where meeting_id=any(ids)),
   'records',(select jsonb_agg(to_jsonb(r)) from attendance_records r where meeting_id=any(ids)));
  insert into attendance_roster(meeting_id,enrollment_id,name,college_id,email)
   select distinct on(enrollment_id) canonical,enrollment_id,name,college_id,email
   from attendance_roster where meeting_id=any(ids) order by enrollment_id,meeting_id
   on conflict do nothing;
  for person in select distinct enrollment_id from attendance_roster where meeting_id=any(ids) loop
   select r.* into chosen from attendance_records r where r.meeting_id=any(ids) and r.enrollment_id=person.enrollment_id
    order by (r.status='present') desc,(r.source='manual') desc,r.updated_at desc limit 1;
   select min(checked_in_at) into earliest from attendance_records where meeting_id=any(ids) and enrollment_id=person.enrollment_id;
   if chosen.meeting_id is not null then
    insert into attendance_records(meeting_id,enrollment_id,status,source,revision,updated_at,checked_in_at)
     values(canonical,person.enrollment_id,chosen.status,chosen.source,chosen.revision+1,chosen.updated_at,earliest)
     on conflict(meeting_id,enrollment_id) do update set status=excluded.status,source=excluded.source,
      revision=greatest(attendance_records.revision,excluded.revision)+1,updated_at=excluded.updated_at,checked_in_at=excluded.checked_in_at;
   end if;
  end loop;
  update attendance_meetings set merged_into=canonical,status='cancelled',token=null,token_slot=null
   where id=any(ids) and id<>canonical;
  insert into attendance_meeting_events(meeting_id,actor_email,action,note)
   select canonical,instructor_email,'merge','Combined duplicate roll calls; original records preserved in merge audit.'
   from attendance_meetings where id=canonical;
 end loop;
end $$;
alter table public.attendance_records enable trigger attendance_check_in_time;
create unique index attendance_one_official_day on public.attendance_meetings(class_id,meeting_date)
 where status<>'cancelled' and merged_into is null;

create or replace function public.attendance_check_in_time() returns trigger language plpgsql set search_path=public as $$
begin
 if tg_op='UPDATE' then
  new.checked_in_at:=old.checked_in_at;
  if old.checked_in_at is null and new.source='check_in' and new.status='present' then new.checked_in_at:=clock_timestamp(); end if;
...
begin
 ...
 if p_resource='classes' and p_action='list' then
  ...
  select coalesce(jsonb_agg(to_jsonb(r)||jsonb_build_object('name',s.name,'class_id',mm.class_id,'meeting_date',mm.meeting_date,'class_name',cc.name)),'[]') into result
   ...
   join attendance_classes cc on cc.id=mm.class_id join attendance_enrollments ee on ee.id=s.enrollment_id where ee.email=p_actor and (ee.google_sub is null or ee.google_sub=p_sub) and mm.status <> 'cancelled' and mm.merged_into is null
  ...
  insert into attendance_records(meeting_id,enrollment_id,status,source) values(m.id,rr.enrollment_id,'present','check_in')
   on conflict(meeting_id,enrollment_id) do update set status='present',source='check_in',revision=attendance_records.revision+1,updated_at=clock_timestamp()
   where attendance_records.status='absent' and attendance_records.source='finalized';
 ...
 if mid is not null then
  ...
  if m.merged_into is not null then raise exception 'This roll call was combined. Reload the official class attendance.'; end if;
 ...
 if p_resource='classes' and p_action='update' then
  ...
  if p_action='open' then
   ...
   n:=clock_timestamp();
   select * into m from attendance_meetings where class_id=c.id and meeting_date=dt and status<>'cancelled' and merged_into is null for update;
   if exists(select 1 from attendance_meetings where instructor_email=p_actor and status='open' and id is distinct from m.id) then raise exception 'End the open class before changing classes'; end if;
   if m.id is null then
    insert into attendance_meetings(class_id,instructor_email,meeting_date) values(c.id,p_actor,dt) returning * into m;
   elsif m.status='closed' then
    update attendance_meetings set status='open',expires_at=n+interval '10 minutes',closed_at=null,extended=false,
     token=null,token_slot=null,reopen_count=reopen_count+1 where id=m.id returning * into m;
    insert into attendance_meeting_events(meeting_id,actor_email,action,note)
     values(m.id,p_actor,'reopen','Reopened for 10 minutes; existing attendance retained.');
   end if;
   insert into attendance_roster(meeting_id,enrollment_id,name,college_id,email)
    select m.id,id,name,college_id,email from attendance_enrollments where class_id=c.id and effective_from<=dt and (effective_to is null or effective_to>=dt) on conflict do nothing;
  ...
  if m.id is null then select * into m from attendance_meetings where class_id=c.id and merged_into is null order by opened_at desc limit 1; else select * into m from attendance_meetings where id=m.id; end if;
...
begin
 ...
 if p_resource='schedule' and p_action<>'list' then
  ...
  if p_action='save' then
   ...
   if p_action='exclude' then
    ...
    for m in select * from attendance_meetings where class_id=cid and meeting_date=dt and status='cancelled' and merged_into is null and not exists(select 1 from attendance_meetings active where active.class_id=cid and active.meeting_date=dt and active.status<>'cancelled') order by id for update loop
 ...
 if p_resource='requests' and p_action in ('create','review') then
  ...
  if m.merged_into is not null then raise exception 'This roll call was combined. Open the official attendance record to request a correction.'; end if;
 ...
 from attendance_meetings mm where mm.class_id=cid and mm.merged_into is null;
