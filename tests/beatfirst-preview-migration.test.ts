...
test('nine-level migration preserves existing progress and private append-only access', async () => {
  const db = new PGlite()
  ...
  try {
    ...
    for (const level of [7, 8, 9]) await insert(level)
    assert.equal((await db.query('select * from public.beatfirst_preview_attempts')).rows.length, 4)
    for (const level of [0, 10]) await assert.rejects(insert(level), /check constraint/)
